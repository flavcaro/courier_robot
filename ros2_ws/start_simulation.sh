...
# Path del workspace (usa il path reale di QUESTO workspace)
...
gnome-terminal -- bash -c "cd '$WS_PATH'; source install/setup.bash; ros2 run courier_nav spawner --ros-args -p use_sim_time:=true; sleep 3; exec bash"
...
# 5. Avvia il controller'$WS_PATH'; source install/setup.bash; ros2 run courier_nav mission_
